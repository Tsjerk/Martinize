...
COLORS = (
    (('ff'), '1;33'),
    (('dssp', 'ss', 'ssc', 'pymol', 'collagen'), '38;5;214'),
    (('f', 'o', 'x', 'n', 'nmap'), '31'),
    (('elastic', 'ef', 'el', 'eu', 'ea', 'ep', 'em', 'eb'), '34'),
    (('link', 'merge', 'cys'), '36'),
)


def color_arguments(arguments, colors):
    """
...
def replace_function(program, arguments, replace_files=True):
    ...
    if func_name is not None:
        ...
        prog_args, input_dir = shlex.split(reduced_name)
        if replace_files:
            arg_list = shlex.split(prog_args)
            new_list = []
            while arg_list:
                arg = arg_list.pop(0)
                new_list.append(arg)
